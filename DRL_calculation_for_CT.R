...
for (CT_model in list_scanner) {
  ...
  print(paste0("The CT protocol number for ", CT_model, " used in ", Study_year ," (before IRSN selection rules) with at least 30 examinations is : ", length(unique(CT_buffer$Series.protocol.name))))
...
for(CT_dataframe in list_CT_model_data_dataframe) {
  ...
  i <- 0 # instance a counter to get the number of protocols analyzed with IRSN specification (french law)
  ...
  print(paste0("The CT protocol number for ", scanner_name," analyzed in 2023 with IRSN selection rules is : ", i))
...
# Retrieve the list of the 10 most frequent examination protocols then we filter on the dataframe
Frequent_protocols_list <- CT_dataframe %>%
  group_by(Series.protocol.name) %>% count(Series.protocol.name) %>% # count the number of occurrences for the descriptions
  arrange(desc(n)) %>% # data is sorted in descending order
  head(10) %>% # select the first 6 lines (i.e. the 6 most frequent)
  pull(Series.protocol.name) # a vector of study descriptions is created
CT_dataframe_protocol_filtered <- CT_dataframe %>% filter(Series.protocol.name %in% Frequent_protocols_list) # filter according to the list of frequent descriptions

# get statistics (mean, sd, max, min) round at unit for 10 more frequent protocols
...
#                                     Mettre une règle de couleur pour savoir si valeur locale sup. ou inf. par rapport aux références nationnales.
